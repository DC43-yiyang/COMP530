...
tuple<MyDB_TableReaderWriterPtr, size_t> LogicalAggregate ::execute() {
  ...
  assert(aggsToCompute.size() + myGroupings.size() == outputSpec->getSchema()->getAtts().size());
...
tuple<MyDB_TableReaderWriterPtr, size_t> LogicalJoin ::execute() {
  ...
  bool leftInSelect = any_of(exprsToCompute.begin(), exprsToCompute.end(), [this](const ExprTreePtr &expr) {
    return any_of(leftTables.begin(), leftTables.end(), [&expr](const pair<string, string> &name) { return expr->referencesTable(name.second); });
  });

  bool rightInSelect = any_of(exprsToCompute.begin(), exprsToCompute.end(), [this](const ExprTreePtr &expr) {
    return any_of(rightTables.begin(), rightTables.end(), [&expr](const pair<string, string> &name) { return expr->referencesTable(name.second); });
  });
  ...
  if (leftInput->getNumPages() < maxPossiblePages || rightInput->getNumPages() < maxPossiblePages) {
    ...
    for (const auto &expr : outputSelectionPredicate) {
      if (expr->getLHS() && expr->getRHS()) {
        ...
        if (any_of(leftAtts.begin(), leftAtts.end(), [&pred1](const pair<string, MyDB_AttTypePtr> &p) { return pred1.find(p.first) != string::npos; }))
...
tuple<MyDB_TableReaderWriterPtr, size_t> LogicalTableScan ::execute() {
  ...
  if (inputSpec->getTable()->getFileType() == "bplustree") {
    ...
    if (recIter) {
      int counter = 0;
      auto temp = inputTable->getEmptyRecord();
      ...
      func predFunc = temp->compileComputation(pred);
